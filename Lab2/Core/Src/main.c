...
int main(void)
{
  ...
  DWT_Init();
    ...
    /* Waveform timing */
    const uint32_t N_SAMPLES = 64;          // samples per period
    const uint32_t SAMPLE_US = 8;           // ~64*8us = 512us period => ~1.95 kHz (audible)
    uint32_t phase = 0;
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  saw      = (phase * 4095u) / (N_SAMPLES - 1u);
	    ...
	    /* Write both outputs.
	       Option A (recommended): one register write updates both channels simultaneously. :contentReference[oaicite:8]{index=8} */
	    //HAL_DACEx_DualSetValue(&hdac1, DAC_ALIGN_12B_R, triangle, saw);

	    /* Option B (if you don’t want DualSetValue):*/
	       HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, triangle);  //:contentReference[oaicite:9]{index=9}
	    //  HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, saw);


	    /* next sample */
	    phase++;
	    if (phase >= N_SAMPLES) phase = 0;

	    DWT_Delay_us(SAMPLE_US);
//	  GPIO_PinState btn = HAL_GPIO_ReadPin(BUTTON_GPIO_Port, BUTTON_Pin);
//
//	  if (btn == GPIO_PIN_RESET)   // active-low: pressed = 0
//	  {
